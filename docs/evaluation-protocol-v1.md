...
This document freezes the experimental protocol for moving from development validation to held-out thesis evaluation. It must be treated as immutable for `evaluation-protocol-v1` once held-out vulnerability testing begins.

The held-out benchmark cases must not be used to tune prompts, mappings, policies, verifier rules, payloads, budgets, discovery behavior, or scanner settings.

## Repository Identity

- Protocol version: `evaluation-protocol-v1`
- Repository commit at protocol preparation: `e5c347c0fbf87591422756a953d5f61c37c9134f`
- Structural validation artifact: `.adstf-runs/heldout-structural-validation-20260719T222131Z/artifacts/heldout-structural-validation.json`
- Held-out manifest: `examples/benchmarks/heldout-manifest-v1.json`
- Held-out ground truth: `examples/benchmarks/heldout-ground-truth-v1.json`

The Git commit identifies the repository base. The following SHA-256 values identify protocol-critical files in the prepared working tree:
...
The held-out benchmark set is local-only and disposable. It is separate from the development benchmarks.

Evaluation units are benchmark cases, not individual HTTP requests. The frozen held-out case set is:
...
Structural validation completed without executing vulnerability tests:

- Run: `.adstf-runs/heldout-structural-validation-20260719T222131Z`
...
This validation proves only that the held-out targets, scopes, reset endpoints, and artifact schemas are ready. It does not produce evaluation results.
